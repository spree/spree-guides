...
NOTE: Spree 1.0.x supports Rails from 3.1.1 to 3.1.4 only.
...
<shell>
rails _3.1.4_ new mystore -d mysql
</shell>

NOTE: Rails version 3.1.4 is delcared explicitly in order to avoid generating the
application with newer Rails 3.2.x which is not compatible.
